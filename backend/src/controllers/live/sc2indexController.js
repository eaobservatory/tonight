...
const getSC2INDEX = async (req, res) => {
  ...
  const rows = await queryJCMT(
    `SELECT backend, obsnum, utdate, project, object, map_wdth, map_hght, tau225st, tau225en, wvmtaust, wvmtauen, seeingst, seeingen, roofsten, date_obs, doorsten FROM SCUBA2 LEFT JOIN COMMON ON SCUBA2.obsid = COMMON.obsid WHERE utdate = ? ORDER BY obsnum DESC`,
    [dateStr]
  );
